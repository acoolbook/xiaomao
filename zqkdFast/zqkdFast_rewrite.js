...
async function getRewrite() {
    ...
    if($request.url.indexOf('FastApi/CommonReward/toGetReward.json') > -1) {//精彩资讯，时段奖励等
        rBody = $request.body
        if(zqkdFastjczxBoxbody) {
            if(zqkdFastjczxBoxbody.indexOf(rBody) > -1) {
                $.msg(jsname+` 此奖励body已存在，本次跳过`)
            } else {
                ...
                $.msg(jsname+` 获取第${bodyList.length}个奖励body成功`)
            }
        } else {
            $.setdata(rBody, 'zqkdFastjczxBoxbody');
            $.msg(jsname+` 获取第1个奖励body成功`)
        }
    }
